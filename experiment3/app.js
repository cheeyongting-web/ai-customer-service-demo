/* ============================================================
   Experiment 3 — Flow controller
   Single-factor 3-level between-subjects: attribution manipulation
     A = effort attribution loading text
     B = neutral loading text
     C = malfunction attribution loading text
   All cells: fixed 3.5s delay + rejection outcome.
   ============================================================ */
(function () {
  "use strict";

  var DATA = window.EXP3_DATA;
  ...
  function ghostBtn(id, label, icon) {
    return (
      '<button id="' + id + '" class="inline-flex items-center justify-center gap-2 bg-white border border-slate-200 text-ink hover:border-primary hover:text-primary font-semibold px-6 py-3 rounded-2xl transition-all active:scale-[.98]">' +
      '<span class="material-symbols-rounded text-[18px]">' +
      (icon || "arrow_back") + "</span>" +
      "<span>" + esc(label) + "</span>" +
      "</button>"
    );
  }
  ...
  function renderConsent() {
    ...
    var html = card(
      stepBadge("参与须知") +
        ...
        '<p class="text-ink-soft leading-relaxed mb-3">您好！感谢您参与本次学术研究。</p>' +
        '<p class="text-ink-soft leading-relaxed mb-3">本调查旨在了解消费者在电商平台使用 AI 客服的真实感受。问卷分为<strong class="text-ink">情境代入</strong>与<strong class="text-ink">问答</strong>两个部分。请您在回答前认真阅读情境描述，并根据您代入情境后的<strong class="text-ink">真实第一反应</strong>进行评价。</p>' +
        '<p class="text-ink-soft leading-relaxed mb-3">本研究采用<strong class="text-ink">匿名形式</strong>，您的个人信息及回答将仅用于学术分析，受到严格保护。</p>' +
        '<p class="text-ink-soft leading-relaxed mb-6">作答时间预计约 <strong class="text-ink">5-8 分钟</strong>，您可以随时退出。</p>' +
        '<div class="space-y-3">' +
  ...
  function consentOption(value, emoji, label) {
    return (
      ...
      '<input type="radio" name="consent" value="' + value + '" />' +
  ...
  function renderScenario() {
    ...
    var html = card(
      stepBadge("情境背景") +
        ...
        esc(cond.scenario) +
        "</p>" +
        '<p class="text-ink-soft">您提交诉求后，系统进入了处理状态，界面显示正在加载……随后 AI 客服将给出回复。</p>' +
        "</div>" +
        '<div class="flex">' +
        primaryBtn("scenario-next", "进入 AI 客服对话", "chat") +
        "</div>"
    );
    mount(html);
  ...
  function renderChat() {
    ...
    sendBtn.addEventListener("click", function () {
      ...
      // Brief 350ms buffer before the loading bubble shows up.
      setTimeout(function () {
        // Show condition-specific loading text with pulsing dots.
        ...
        typingMsg.innerHTML =
          ...
          '<span>' + esc(cond.loading_text) + '</span>' +
        ...
        // Wait fixed delay (3.5s) then show the rejection reply.
        setTimeout(function () {
          typingMsg.remove();
  ...
  function likertBlock(item, mod) {
    ...
    return (
      ...
      '<p class="text-[15px] leading-relaxed mb-4 font-medium">' + esc(item.text) + "</p>" +
      '<div class="flex items-center justify-between gap-1 sm:gap-2">' + dots + "</div>" +
  ...
  function singleBlock(item) {
    var opts = item.options
      .map(function (opt) {
        return (
          '<label class="choice-opt block cursor-pointer">' +
          '<input type="radio" name="' + item.key + '" value="' + esc(opt) + '" />' +
    ...
    return (
      '<div class="border border-slate-100 rounded-2xl p-4 sm:p-5 bg-white/70">' +
      '<p class="text-[15px] leading-relaxed mb-4 font-medium">' + esc(item.text) + "</p>" +
      '<div class="grid grid-cols-1 sm:grid-cols-2 gap-2.5">' + opts + "</div>" +
      "</div>"
    );
  }
  ...
  function buildSubmission() {
    ...
    // Attention-check pass flag (must be "不同意" = the second option)
    var imc_pass = a.IMC1 === "不同意" ? 1 : 0;

    // Auto Name: cell_shortId_timestamp
    var now = new Date();
    var ts =
    ...
    return {
      ...
      experiment: "3",
      cell: cond.cell,
      attribution_cond: cond.attribution_cond,
      loading_text: cond.loading_text,
      delay_ms: String(cond.delay_ms),

      // Attribution manipulation check
      AttrEffort1: String(a.AttrEffort1),
      AttrEffort2: String(a.AttrEffort2),
      AttrFault1: String(a.AttrFault1),
      AttrFault2: String(a.AttrFault2),

      // Core DV: Satisfaction with recovery
      SA1: String(a.SA1),
      SA2: String(a.SA2),
      SA3: String(a.SA3),
      ...
      // Auxiliary
      SpeedPerc: String(a.SpeedPerc),
      Impatience: String(a.Impatience),

      // Attention check
      IMC1: a.IMC1,
      imc_pass: String(imc_pass),
  ...
  function submitAndFinish() {
    ...
    submitToAirtable(payload)
      ...
      .catch(function (err) {
        ...
        try {
          var key = "exp3_offline_" + state.participant_id;
  ...
  function getAirtableApiKey() {
    ...
    if (!apiKey) {
      ...
      if (apiKey) {
        try { localStorage.setItem("airtable_key", apiKey); } catch (err) { void err; }
      }
    }
    return apiKey || DATA.AIRTABLE.apiKey || "";
  }

  function getAirtableTarget() {
    var baseId = "";
    var tableId = "";
    try {
      baseId = localStorage.getItem("exp3_airtable_base") || "";
      tableId = localStorage.getItem("exp3_airtable_table") || "";
    } catch (err) { void err; }
    return {
      baseId: baseId || DATA.AIRTABLE.baseId || "",
      tableId: tableId || DATA.AIRTABLE.tableId || ""
    };
  }

  function submitToAirtable(fields) {
    var target = getAirtableTarget();
    var apiKey = getAirtableApiKey();

    // Filter out empty values so Airtable typecast doesn't choke.
    var clean = {};
    ...
    if (!target.baseId || !target.tableId) {
      console.warn(
        "[Experiment3] Airtable base/table not configured — storing payload offline.",
        clean
      );
      return Promise.reject(new Error("Airtable base/table not configured"));
    }

    if (!apiKey || apiKey === "YOUR_AIRTABLE_API_KEY") {
      console.warn(
        "[Experiment3] Airtable API key not configured — storing payload offline.",
        clean
      );
    ...
    var url =
      "https://api.airtable.com/v0/" + target.baseId + "/" + target.tableId;
